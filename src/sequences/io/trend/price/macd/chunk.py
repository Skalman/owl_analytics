...
from techmodels.indicators.trend.price.macd import macd


def macd_chunk(data,
               nfast=10, nslow=35, nema=5,
               return_all=False, getter=lambda x: x):
    prices = numpy.array(map(getter, data))

    calculated = macd(
        prices, nfast=nfast, nslow=nslow, nema=nema, return_all=True)
    y1 = calculated['y1']

    '''
    Skip the first nfast values

    Note: could use a list view instead to save memory, but it might be less
    efficient
    See:
    http://stackoverflow.com/questions/3485475/can-i-create-a-view-on-a-python-list#3485490
    '''
